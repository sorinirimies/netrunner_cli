#!/usr/bin/env nu
# Nightly dependency upgrade for netrunner_cli.
#
# Phases:
#   1. cargo upgrade --incompatible allow
#      Rewrites version pins in [dependencies] so libs like
#      `clap = "4.6"` and `tokio = "1.50"` can advance to newer releases.
#   2. cargo update
...
def main [
    ...
    print $"($cyan)════════════════════════════════════════($reset)"
    print $"($cyan)  netrunner_cli — Nightly Dependency Upgrade($reset)"
    print $"($cyan)════════════════════════════════════════($reset)"
    print ""

    # ── Phase 1: Upgrade Cargo.toml pins ─────────────────────────────────────
    # cargo-edit's `cargo upgrade` rewrites the version strings in Cargo.toml
    # [dependencies].  --incompatible allow lifts the major-version guard so
    # `clap = "4.6"` can move to "5.0", etc.
    ...
    let clippy = (run_check "clippy " { run-external "cargo" "clippy" "--all-targets" "--all-features" "--" "-D" "warnings" "-A" "deprecated" })
    let tests  = (run_check "tests  " { run-external "cargo" "test" "--all-features" "--all-targets" })
